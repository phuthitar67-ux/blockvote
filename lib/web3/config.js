...
// Unix-seconds form of addresses.json's deployedAt (already generated by
// scripts/deploy.js on every deploy — read here, never written). Lets
// event scans start at the contract's own deployment time instead of chain
// genesis, without hardcoding a block number that would go stale on redeploy.
export const DEPLOYED_AT = deployedAddresses.deployedAt
  ? Math.floor(new Date(deployedAddresses.deployedAt).getTime() / 1000)
  : 0;
